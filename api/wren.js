...
- Custom workout from a list she gives you: Lauren sometimes has a workout written down (e.g. from an injury/vacation week, or anything outside the normal 3-session program) and just wants to hand you the exercise names. When she does — whether or not she frames it as "for injury week" — call bloom_actions with create_custom_workout, name (a short name for it — ask if she didn't give one), and exercises (the exercise names, in the exact order she listed them). If she says two exercises are done as a superset, add that pair to supersets (e.g. supersets: [["Lateral Raise", "Tricep Pushdown"]] — every name in a pair must also be in exercises). This saves it to her workout library (same place "Build a workout" saves to) — it does NOT touch or replace the 12-week program, and it shows up automatically in the injury/vacation card and the Workout Library page in Settings. If her list is long or an exercise name is ambiguous, read it back to confirm before saving.
- Renaming or reordering saved workouts: these are the same library entries create_custom_workout writes to, shown on the home screen's injury/vacation card and the Workout Library page. If Lauren asks to rename one, call bloom_actions with rename_workout, name (its current name, exactly), and new_name. If she asks to reorder them (e.g. "move Push Day to the top", "put Leg Day last"), call bloom_actions with reorder_workouts and order — the FULL list of every workout's name, in the new order (the current names/order are in workoutNames in the context block). Getting the whole list right matters — a name you drop from order falls to the bottom rather than disappearing, but don't rely on that; include everything.
        ...
        properties: {
          actions: {
            ...
            items: {
              type: 'object',
              properties: {
                type: { type: 'string', description: 'Action type. Program/chat scope: generate_program, assign_punishment, flag_plateau, set_schedule, edit_workout, apply_deload, remove_deload, mark_injured (flag a program week as an injury week — pair with week_number), unmark_injured (clear that flag — pair with week_number), skip_session (mark one lifting session A/B/C as intentionally skipped this week — pair with session_label, optional week_number, optional reason), unskip_session (clear a skip — pair with session_label, optional week_number), add_cardio_session (week-scoped, Lauren-triggered cardio — pair with `name` and `day`), create_custom_workout (save a workout Lauren dictated to her library — pair with `name`, `exercises`, optional `supersets`), rename_workout (pair with `name` = current name, `new_name` = new name), reorder_workouts (pair with `order` = full list of workout names in the new order), remember (save a long-term fact about Lauren), forget_note (drop one). Live-workout scope (only valid when the user message says it is mid-workout): set_target_weight, set_target (rep target), set_rest, add_set, add_exercise, remove_exercise, reorder, superset (group two current exercises for this session — pair superset_a + superset_b), unlink_superset (ungroup — pass the exercise name).' },
                ...
                order: { type: 'array', items: { type: 'string' }, description: 'For live-workout reorder: list of exercise names in the desired new order. For edit_workout reorder: same shape — full list of exercise names for the session in their new order (must name every exercise currently in the session). Applies across all 12 weeks. For reorder_workouts: full list of saved workout names (from workoutNames in the context block) in the new display order.' },
                ...
                name: { type: 'string', description: 'For add_cardio_session: the cardio name as Lauren would say it (e.g. "Spin class", "HIIT", "Run", "Yoga"). Keep it short and human. For create_custom_workout: a short name for the workout. For rename_workout: the workout\'s CURRENT name, exactly as it appears in workoutNames — pair with new_name.' },
                new_name: { type: 'string', description: 'For rename_workout: the new name for the workout.' },
                ...
                exercises: { type: 'array', items: { type: 'string' }, description: 'For create_custom_workout: the exercise names, in order, exactly as Lauren listed them.' },
                supersets: {
                  type: 'array',
                  description: 'For create_custom_workout: groups of exercise names (each must also appear in `exercises`) that are done as a superset. Omit or send an empty array if none.',
